  ...
  if (i(command, "station")) { //station
    if (i(args[0], "set")) { //station set
      if (i(args[1], "mining")) { //station set mining
        await player.msg("Adding a mining station..").catch((err) => console.error(err));
        try {
              ...
              if (playerFaction) { // If the player is not in a faction, this will return null
                await addMiningStation(playerEntity, playerFaction, function (err, result) {
                  if (err) {
                    ...
                    return player.msg("There was an error adding the station!  Please try again!").catch((err) => console.error(err));
                  } else {
                    return player.msg("Added the station as a mining rig!").catch((err) => console.error(err));
                  }
                });
              } else {
                await player.msg("You must be in a faction to designate a station as a mining facility!  Please join a faction first!").catch((err) => console.error(err));
              }
            } else {
              await player.msg(`Error:  You may only designate a station as a mining facility!  You are in a ${entityType} not a station!`).catch((err) => console.error(err));
              await player.msg("Please enter a build block for the station you would like to designate.").catch((err) => console.error(err));
            }
          } else {
            await player.msg("To designate a station as a mining facility, you must be in control of the station.  Please enter a build block for the station you would like to designate.").catch((err) => console.error(err));
          }
        } catch (err) {
      ...
      var playerFactionObj = await player.faction();
      var playerCurrentEntity = await player.currentEntity();
      var playerCurrentEntityType;
      if (playerCurrentEntity) {
        ...
        if (playerCurrentEntityType == "station") {
          ...
          if (miscHelpers.existsAndIsFile(stationFile)) {
            ...
            return fs.readFile(stationFile, 'utf8', function (err, result) {
              if (err) {
                throw err;
              }
              ...
              if (storageList) { // This needs to be here since you've decided to use "null" as a temporary value rather than an empty array
                var itemNumber; // Don't declare new variables for each for loop, reuse the same ones.
                var itemCount;
                var promiseArray = [];
                ...
                Promise.all(promiseArray).then(function (values) { // This will wait till all PlayerObj.giveId methods have finished.
                  ...
                  if (success && !failure) {
                    ...
                    player.msg("There were no items to give at this time!  Please try again later!").catch((err) => thisConsole.error(err));
                  } else if (success) {
